...
export async function POST(request: Request) {
  try {
    ...
    const { email, type, name, phone, dob, city, education } = body as {
      email: string;
      type: "signup" | "login";
      name?: string;
      phone?: string;
    ...
    // For signup: use registration data from request body
    const regData: { phone?: string; dob?: string; city?: string; education?: string } =
      type === "signup" ? { phone, dob, city, education } : {};
